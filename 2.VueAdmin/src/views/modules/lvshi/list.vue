<template>
	<div class="main-content" :style='{"color":"#666","padding":"0px 30px 30px","fontSize":"14px"}'>
			...
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","margin":"0 0 10px","borderRadius":"0 0 8px 8px","textAlign":"left","background":"#fff","display":"block","width":"100%","fontSize":"inherit","order":"1"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">律师账号</label>
						<el-input v-model="searchForm.lvshizhanghao" placeholder="律师账号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">律师姓名</label>
						<el-input v-model="searchForm.lvshixingming" placeholder="律师姓名" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="律师等级" prop="lvshidengji">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">律师等级</label>
						<el-select clearable v-model="searchForm.lvshidengji" placeholder="请选择律师等级" >
							<el-option v-for="(item,index) in lvshidengjiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="专业领域" prop="zhuanyelingyu">
						...
						<el-select clearable v-model="searchForm.zhuanyelingyu" placeholder="请选择专业领域" >
							<el-option v-for="(item,index) in zhuanyelingyuOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="地区" prop="diqu">
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit","order":"2"}'>
					<el-button class="add" v-if="isAuth('lvshi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('lvshi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('lvshi','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-shenhe2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						审核
					</el-button>

					<el-button class="btn18" v-if="isAuth('lvshi','律师人数')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						律师人数
					</el-button>
					<el-button class="btn18" v-if="isAuth('lvshi','律师等级')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						律师等级
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('lvshi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('lvshi','在线预约')" @click="zaixianyuyueCrossAddOrUpdateHandler(scope.row,'cross','是','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								在线预约
							</el-button>
							<el-button class="edit" v-if=" isAuth('lvshi','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('lvshi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<zaixianyuyue-cross-add-or-update v-if="zaixianyuyueCrossAddOrUpdateFlag" :parent="this" ref="zaixianyuyueCrossaddOrUpdate"></zaixianyuyue-cross-add-or-update>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="lvshidengjiChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
import zaixianyuyueCrossAddOrUpdate from "../zaixianyuyue/add-or-update";
	export default {
		data() {
			return {
				lvshidengjiOptions: [],
				zhuanyelingyuOptions: [],
				diquOptions: [],
				searchForm: {
					key: ""
				},
				...
				zaixianyuyueCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			zaixianyuyueCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.zaixianyuyueCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','lvshi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.zaixianyuyueCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.zaixianyuyueCrossaddOrUpdate.init(row.id,type);
				});
			},
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var lvshidengjiChart2 = echarts.init(document.getElementById("lvshidengjiChart2"),'macarons');
        this.$http({
            url: "lvshi/group/lvshidengji",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                let pArray = []
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].lvshidengji);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].lvshidengji
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                lvshidengjiChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    lvshidengjiChart2.resize();
                };
            }
        });
      })
    ...
    init () {
        this.sfshOptions = "是,否,待审核".split(',');
          ...
          this.$http({
            ...
            if (data && data.code === 0) {
              this.zhuanyelingyuOptions = data.data;
            } else {
		...
		this.$refs["shBatchForm"].validate(valid => {
			if(valid){
				this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
					...
					this.$http({
						url: "lvshi/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
						method: "post",
						data: this.batchIds
					}).then(({ data }) => {
						if (data && data.code === 0) {
