    ...
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="资源分类" prop="ziyuanfenlei">
            <el-input v-model="ruleForm.ziyuanfenlei" 
                placeholder="资源分类" clearable :disabled=" false  ||ro.ziyuanfenlei"></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`ziyuanfenlei/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('ziyuanfenlei/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`ziyuanfenlei/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
