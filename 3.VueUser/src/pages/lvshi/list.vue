...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px 0","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","display":"flex","width":"100%","borderStyle":"solid","height":"auto","order":"2"}'>
      <el-form-item :style='{"margin":"0 5px 0px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>律师账号：</div>
        <el-input v-model="formSearch.lvshizhanghao" placeholder="律师账号" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 5px 0px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>律师姓名：</div>
        <el-input v-model="formSearch.lvshixingming" placeholder="律师姓名" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('lvshi','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 15px","margin":"0px 0px 0 0","color":"#fff","display":"inline-block","outline":"none","borderRadius":"30px","background":"#1c6a6d","width":"90px","fontSize":"14px","lineHeight":"40px","height":"40px"}' type="primary" @click="add('/index/lvshiAdd')"><i v-if="false" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="sort_view" :style='{"padding":"10px 20px","margin":"26px 0 0","borderColor":"#3493c3","overflow":"hidden","borderRadius":"0px","textAlign":"right","background":"none","borderWidth":"0 0 3px","width":"100%","borderStyle":"solid","order":"3"}'>
		<el-button :style='{"border":"0","padding":"4px 5px","margin":"0 5px","borderRadius":"0px","background":"none"}' @click="sortClick('clicknum')">
		...
		<el-button :style='{"border":"0","padding":"0 5px","margin":"0 5px","borderRadius":"8px"}' @click="sortClick('storeupnum')">
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#929292"}' class="icon iconfont icon-xiaoliang13" v-if="sortType!='storeupnum'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#929292"}' class="icon iconfont icon-jiantou23" v-else-if="sortType=='storeupnum'&&sortOrder=='desc'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#929292"}' class="icon iconfont icon-jiantou24" v-else-if="sortType=='storeupnum'&&sortOrder=='asc'"></span>
			<span :style='{"color":"#929292","lineHeight":"40px","fontSize":"14px"}'>收藏数</span>
		</el-button>
	</div>
	<div class="list" :style='{"minHeight":"699px","padding":"20px 0","margin":"20px 0 0","borderRadius":"0px","background":"none","flex":"1","width":"calc(100% - 310px)","float":"left","order":"5"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0 0 0 2%","color":"#999","flexWrap":"wrap","background":"none","display":"flex","width":"100%","fontSize":"14px","height":"auto"}'>
			<div :style='{"border":"0px solid #ddd","cursor":"pointer","padding":"0 0 0px","boxShadow":"1px 2px 9px #ccc","margin":"0 2% 30px 0","display":"flex","justifyContent":"flex-end","overflow":"hidden","flexWrap":"wrap","background":"#fff","width":"23%","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<img @click.stop="imgPreView(item.touxiang)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"240px"}' v-if="item.touxiang && item.touxiang.substr(0,4)=='http'" :src="item.touxiang.split(',')[0]" class="image" />
				...
				<div :style='{"padding":"0 10px","borderColor":"#ddd","margin":"0 0 6px","whiteSpace":"nowrap","color":"#333","textAlign":"left","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"32px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"dashed"}' class="name ">{{item.lvshixingming}}</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.selectOptionsList.push({name:'律师等级',list:this.lvshidengjiOptions,tableName: 'lvshidengji',check: -1})
		this.$http.get('option/zhuanyelingyu/zhuanyelingyu').then(res => {
			if (res.data.code == 0) {
				this.zhuanyelingyuOptions = res.data.data;
				this.selectOptionsList.push({name:'专业领域',list:this.zhuanyelingyuOptions,tableName: 'zhuanyelingyu',check: -1})
			}
		});
		this.$http.get('option/diquxinxi/diqu').then(res => {
			if (res.data.code == 0) {
				this.diquOptions = res.data.data;
				this.selectOptionsList.push({name:'地区',list:this.diquOptions,tableName: 'diqu',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.lvshizhanghao != '') searchWhere.lvshizhanghao = '%' + this.formSearch.lvshizhanghao + '%';
        if (this.formSearch.lvshixingming != '') searchWhere.lvshixingming = '%' + this.formSearch.lvshixingming + '%';
        if (this.formSearch.lvshidengji != '') searchWhere.lvshidengji = this.formSearch.lvshidengji;
        ...
        this.$http.get(`lvshi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/lvshiDetail', query: params});
      },
