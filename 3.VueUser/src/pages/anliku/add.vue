...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#edeff1"}'>
    <el-form
      ...
      label-width="150px"
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="案例名称" prop="anlimingcheng">
            <el-input v-model="ruleForm.anlimingcheng" 
                placeholder="案例名称" clearable :disabled=" false  ||ro.anlimingcheng"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="案例类型" prop="anlileixing">
            <el-select v-model="ruleForm.anlileixing" placeholder="请选择案例类型" :disabled=" false  ||ro.anlileixing" >
              <el-option
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="相关律师" prop="xiangguanlvshi">
            <el-input v-model="ruleForm.xiangguanlvshi" 
                placeholder="相关律师" clearable :disabled=" false  ||ro.xiangguanlvshi"></el-input>
          </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/anlileixing/anlileixing', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`anliku/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('anliku/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`anliku/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
